...
// ---- Pages 2+: the proof behind the numbers -------------------------------
const photoPages = [
  { file: "cs-mon.jpg", label: "Count Sheet — Mon 7/27 — Citrus" },
  { file: "cs-sat.jpg", label: "Count Sheet — Sat 8/1 — Mesa" },
  { file: "tc-week.jpg", label: "Time Card — Week of 7/27" }
];
let pageNo = 1;
for (let i = 0; i < photoPages.length; i += 2) {
  ...
  doc.addImage(logoData, "PNG", 28, 18, 30, 30);
  doc.setFont("helvetica", "bold").setFontSize(11).setTextColor(0);
  doc.text("EDWIN BONILLA — Week 7/27/2026 - 8/1/2026 — Attachments", 66, 37);
  const slots = photoPages.slice(i, i + 2);
  slots.forEach((slot, idx) => {
    const y = 60 + idx * ((ph - 90) / 2);
    doc.setFont("helvetica", "bold").setFontSize(10).setTextColor(40);
    doc.text(slot.label, 28, y);
    const img = "data:image/jpeg;base64," + fs.readFileSync(slot.file).toString("base64");
    const boxW = pw - 56;
    const boxH = (ph - 90) / 2 - 40;
    // keep the photo's 1000x700 shape inside the box
    const scale = Math.min(boxW / 1000, boxH / 700);
    doc.addImage(img, "JPEG", 28, y + 8, 1000 * scale, 700 * scale);
  });
}
...
for (let p = 1; p <= pages; p++) {
  ...
  doc.text(`Page ${p} of ${pages}`, w / 2, h - 14, { align: "center" });
}

fs.writeFileSync("mgp-week-packet-MOCKUP.pdf", Buffer.from(doc.output("arraybuffer")));
console.log("written");
